# Плагин Tags Color Files

[English](https://github.com/pan4ratte/obsidian-tags-color-files/blob/main/README.md) | Русский

Данный плагин позволяет автоматически выделять файлы в вашем проводнике Obsidian разными цветами на основании имеющихся в них тегах. Цвета файлов определяются правилами, которые вы создаёте в настройках плагина, вводя тег и присваивая ему цвет. После добавления к файлу тега, для которого создано правило, этот файл окрашивается в соответствующий цвет в проводнике Obsidian.

![](media/plugin-demo-settings.png)


## Фичи

1. Автоматическое выделение файлов в проводнике Obsidian на основании правил типа тег-цвет, которые вы создаёте.

2. Создание правил для окрашивания файлов внутри отдельных папок.

3. Приоритизация тегов в настройках плагина. Полезно в тех случаях, когда в одном файле содержится больше одного тега, для которого создано правило окрашивания. Плагин выберет тег с большим приоритетом для окрашивания такого файла.

4. Обратное выделение. По умолчанию каждое новое правило выделяет файлы с указанным тегом, но вы можете инвертировать логику правила и выделять файлы, не содержащие указанный вами тег.

5. Выбор способов окрашивания: не только базовые, такие как как "Текст" или "Фон", но и расширенные, такие как "Точки перед/после текста" и "Точки перед/после текста + текст". Также доступна настройка размеров цветных точек. При выборе расширенных вариантов, в проводнике одновременно отображается до трёх цветных точек если в одном файле содержится несколько тегов, для которых создано правило окрашивания.

6. Экспорт и импорт настроек плагина в файл.

7. Поддержка локализации. Языки, доступные на данный момент: английский и русский.


## Кейс использования плагина

Изначально, плагин был создан для моих личных нужд. При чтении литературы я помечаю некоторые цитаты тегами по типу `#ключевое` или `#не-согласен`, когда хочу выделить соответствующие мысли. Мне было бы более удобно видеть такие цитаты в проводнике Obsidian без необходимости фильтровать заметки в строке поиска или в панели тегов. — Именно эту проблему решает данный плагин, но я могу представить себе и другие намного более творческие способы его использования.


## Установка

### Вариант 1: Магазин плагинов Obsidian

1. Находясь в настройках Obsidian, откройте вкладу "Сторонние плагины" и нажмите на кнопку "Обзор".

2. В поисковой строке введите `Tags Color Files`, нажмите на результат, затем на кнопки "Установить" и "Включить".

Альтернативно, вы можете установить плагин, перейдя на сайт сообщества по ссылке: [https://community.obsidian.md/plugins/tags-color-files](https://community.obsidian.md/plugins/tags-color-files)

### Вариант 2: плагин BRAT

Если вы хотите протестировать бета-версию плагина или предыдущие обновления, вы можете сделать это с помощью плагина `BRAT`:

1. Из официального магазина плагинов Obsidian установите плагин `BRAT`.

2. В настройках `BRAT` найдите раздел "Beta plugin list" и нажмите на кнопку "Add beta plugin".

3. В появившемся окне вставьте ссылку на репозиторий плагина `Tags Color Files`: [https://github.com/pan4ratte/obsidian-tags-color-files](https://github.com/pan4ratte/obsidian-tags-color-files)

4. В пункте "Select a version" выберите нужную версию и нажмите на кнопку "Add plugin". Плагин автоматически установится и будет готов для использования.


## Коллаборация и Дорожная карта

Я открыт к предложениям и новым идеям, расширяющим функционал плагина. Ниже перечислены запланированные обновления — некоторые из них я не могу имплементировать самостоятельно (я не программист), но они были бы отличными "quality of life" улучшениями.

- [x] ~~Визуальное улучшение всех методов окрашивания с точками. Сейчас я не совсем доволен тем, как они интегрированы в интерфейс Obsidian. Их нужно обновить так, чтобы они не конфликтовали с другими элементами интерфейса.~~

- [x] ~~Оптимизация плагина для Obsidian на телефонах.~~

- [x] ~~Появление ошибки, если пользователь вводит тег с некорректным форматированием при создании нового правила окрашивания.~~

- [x] ~~Предотвращение создания двух правил для одного и того же тега.~~

- [x] ~~Подсказки существующих тегов в текстовом поле при создании нового правила.~~

- [x] ~~Опция выбора между тремя размерами точек для методов окрашивания "Точки ..."~~

- [ ] Опция экспорта на мобильных устройствах. Сейчас она доступна только на компьютерах и была удалена из мобильной версии плагина из-за сложностей реализации.

### Известные баги

- [x] ~~При определённых условиях, перетаскивание тега в настройках может привести к удалению этого тега из базы данных плагина~~


## Об авторе

Меня зовут Марк Ингрэм, я религиовед, и кроме своего основного направления исследований (протестантская политическая теология в России), я преподаю предмет "Информационные технологии в научных исследованиях" на основании своей собственной уникальной программы. Данный плагин помогает мне в исследованиях, а также я использую его в преподавании, как и другие плагины, которые я разрабатываю и которые вы можете найти в моём профиле на GitHub.

Привет всем студентам, которые зашли на эту страницу!
